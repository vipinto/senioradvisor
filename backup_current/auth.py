...
async def register_user(email: str, password: str, name: str, db: AsyncIOMotorDatabase):
...
async def _ensure_roles(user: dict, db: AsyncIOMotorDatabase) -> dict:
    """Lazy migration: ensure user has roles array and active_role"""
    if "roles" not in user or not user.get("roles"):
        role = user.get("role", "client")
        roles = [role] if role != "admin" else ["client"]
        active_role = roles[0]
        await db.users.update_one(
            {"user_id": user["user_id"]},
            {"$set": {"roles": roles, "active_role": active_role}}
        )
        user["roles"] = roles
        user["active_role"] = active_role
    if "active_role" not in user:
    ...
    if bearer_token:
        try:
            ...
            if payload.get("type") == "jwt":
                ...
                if user:
                    user = await _ensure_roles(user, db)
...
async def require_provider(user: dict, db: AsyncIOMotorDatabase):
    if "provider" not in user.get("roles", []) and user.get("role") != "provider":
...
async def require_admin(user: dict):
    if user.get("role") != "admin" and "admin" not in user.get("roles", []):
