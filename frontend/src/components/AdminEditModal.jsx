...
export default function AdminEditModal({ section, provider, onClose, onSaved }) {
  ...
  const [formData, setFormData] = useState(() => {
    switch (section) {
      ...
      case 'settings':
        return {
          is_featured: provider.is_featured_admin || provider.is_featured || false,
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const endpoint = type === 'gallery'
          ...
          : `/admin/providers/${provider.provider_id}/slider/upload`;
  ...
  const handleDeletePremiumPhoto = async (photoId) => {
    try {
      await api.delete(`/admin/providers/${provider.provider_id}/slider/${photoId}`);
      toast.success('Foto eliminada');
      onSaved();
    } catch (e) {
      toast.error(e.response?.data?.detail || 'Error al eliminar');
        ...
        <div className="p-6 space-y-4">
          ...
          {section === 'premium_gallery' && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(provider.slider_photos || []).map((photo, i) => (
              ...
              {(provider.slider_photos || []).length < 10 && (
                <div>
                  <input
                  ...
                  >
                    {uploading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Upload className="w-4 h-4 mr-2" />}
                    Subir Foto ({(provider.slider_photos || []).length}/10)
                  </Button>
                </div>
              )}
            </>
